...
function formatOper(val,row,index){
	var id = row.id,links;
	links =  '<a href="#" onclick="removeRecordByOne('+id+',1)">删除</a>    <a href="#" onclick="rename('+index+')">重命名</a>    <a href="#" onclick="showApplyView('+index+')">下载</a>'; 
	return links;
}  
//列表勾选框事件
...
function batchDownload(){
	...
	if(confirm("确定要下载选中的数据吗？")){
		$("#ids").val(ids); 
		$('#apply_audio_forms').form('submit', {
            onSubmit: function(){ 
            },
            success:function(data){
            	var v = eval('(' + data + ')');
...
function rename(index){
	$('#file_upload_list').datagrid('selectRow',index);
	var row = $('#file_upload_list').datagrid('getSelected');
	if (row) {
		$('#renameFile_fm').form('load', row);
	    $('#renameFile').dialog('open');
	   	var fileName = $("#fileName").val();
	    $("#newFileName").val(fileName);
	    /* $('#renameFile_fm').form('clear'); */
	}else if(row=='null'){
...
function saveFileName(){
	...
	var newFileNameSuffix = "." + newFileName.replace(/.+\./, "");
	var fileName = $("#fileName").val();
	var fileNameSuffix = "." + fileName.replace(/.+\./, "");
	var flag = true;
	if (checkSubmit()) {
		$('#submit').linkbutton({  
		    disabled:true
	 ...
	 if(newFileNameSuffix!=fileNameSuffix){  
         flag=false;  
     }else{
    	 flag=true;  
     }
	 if(flag){   
    		$('#renameFile_fm').form('submit', {
    			url:"${ctx}/storageFile/fileRename", 
    			onSubmit : function() {
    			},
    			success : function(result) {
    				...
    				$('#file_upload_list').datagrid('reload');
    				$('#renameFile').dialog('close');
					/* window.location.href = '${namespacePath}/'; */
    			}
    		});
     } 
 else{  
...
function removeRecordByOne(id,status) {
	if(confirm("确定要删除选中的数据吗？")){
		$.post('${ctx}/storageFile/removeRecord?ids='+id, {
			status:status
		}, function(result) {
			if (result.status){
				$('#file_upload_list').datagrid('reload');
...
function removeRecord(status) {
	...
	if(confirm("确定要删除选中的数据吗？")){
		$.post('${ctx}/storageFile/removeRecord?ids='+ids, {
			status:status
		}, function(result) {
			if (result.status){
				$('#file_upload_list').datagrid('reload');
...
function upload(){
	var cname = escape(encodeURIComponent('${customerName}'));
	var cbp = escape(encodeURIComponent('${certifyBusinessDepart}'));
	window.open('${ctx}/upload/fileDetailForUpload?customerName='+cname+'&customerNo='+'${customerNo}'+'&applicationCaseNo='+'${applicationCaseNo}'+'&certifyBusinessDepart='+'${cbp}'+'&loanDateStr='+'${loanDateStr}'+'#');
}
</script>
<body class="easyui-layout">
    ...
    <div data-options="region:'center'" style="padding:5px;">
		...
		<div border="false" style="overflow:hidden; border-bottom:1px solid #ccc;" >
			<div style="padding: 5px; border-bottom:1px solid #f2f3f5">
				...
				<span  id="uploadBtn" class="easyui-linkbutton" iconCls="icon-upload" plain="true" onclick="batchDownload();">批量下载</span> 
				<span  id="uploadBtn" class="easyui-linkbutton" iconCls="icon-upload" plain="true" onclick="upload();">上传</span> 
		...
		<!-- 以下列表 -->
		<div border="false">
			<table id="file_upload_list" class="easyui-datagrid" width="auto" url="${ctx}/upload/getUploadRecordsForXinD?applicationCaseNo=${applicationCaseNo}" title="文件上传列表" rownumbers="true" fitColumns="true" singleSelect="true"  >
				<thead>
					<tr >
						<th field="id" width="10px" align="center"  formatter="controllIfSelected" ></th>
	...
	<!-- 文件重命名-->
	 <!-- <div id="renameFile"  title ="文件重命名 " class="easyui-dialog" style="margin-top:0px;text-align:center;width:400px;height: 300px" closed="true" > -->
<div id="renameFile" class="easyui-window" title="文件重命名" closed="true" style="width:500px;height:300px" data-options="iconCls:'icon-upload',modal:true">			
				<form id="renameFile_fm" method="post">
				<table  style="border-collapse:separate; border-spacing:0px 10px;table-layout:fixed;">
					<tr >
	      				<td align="left"><label class="bitian">文件名:&nbsp&nbsp&nbsp&nbsp</label></td>
	      				<td  align="left">
						<input type="text" id="newFileName" style="width:200px"  size="49" name="newfileName"  required="true"/>
						<input type="hidden" id="fileName" style="width:150px"  size="49" name="fileName"/>
						<input type="hidden" style="width:150px"  size="49" name="id"/>
						</td>
			   		</tr>
			   		<tr>
			   			<td align="left">
			   			<input type="button" align="center"  onclick="saveFileName()" value="提交" />
			   			</td>
			   			<td align="left">
			   		...
			   		<tr>
			   			<td align="left"  class="red" id="tip1"></td>
			   		</tr>
	...
	<!-- 上传按钮悬浮DIV -->
	<div id="mydiv1" style="position:fixed;display:none;border:1px solid silver;background:silver;left:350px;top:137px;">
		<a href="#" id="apply_button" class="easyui-linkbutton" iconCls="icon-upload" plain="true" onclick="openUpload('1');" >上传视频</a>
	</div> 
	<div id="mydiv2" style="position:fixed;display:none;border:1px solid silver;background:silver;left:350px;top:137px;">
		<a href="#" id="apply_button" class="easyui-linkbutton" iconCls="icon-upload" plain="true" onclick="openUpload('2');" >联系人核实</a><br>
		<a href="#" id="apply_button" class="easyui-linkbutton" iconCls="icon-upload" plain="true" onclick="openUpload('3');" >银行流水核实</a><br>
		<a href="#" id="apply_button" class="easyui-linkbutton" iconCls="icon-upload" plain="true" onclick="openUpload('4');" >保单贷核实</a><br>
		<a href="#" id="apply_button" class="easyui-linkbutton" iconCls="icon-upload" plain="true" onclick="openUpload('5');" >随车贷核实</a><br>
		<a href="#" id="apply_button" class="easyui-linkbutton" iconCls="icon-upload" plain="true" onclick="openUpload('6');" >水电煤单据核实</a><br>
		<a href="#" id="apply_button" class="easyui-linkbutton" iconCls="icon-upload" plain="true" onclick="openUpload('7');" >其他核实</a>
	</div>
	<form id="apply_audio_form" method="get" action="${ctx}/upload/getAudioApplyDesc">
		<input hidden="true" name="fileUploadId" id="uploadId">
	</form>
	<form id="apply_audio_forms" method="post" action="${ctx}/upload/getApplyDescs">
